...
\newcommand{\TheoryCompatibleCommit}{f14befe}
\newcommand{\TheoryGenerationDate}{2026-06-27}
\newcommand{\TheoryPipelineVersion}{H3-W9b}
\newcommand{\TheoryImplementationStatus}{implemented through H3-W9b POWHEG real-smoke and real-free execution}
...
\vfill
{\large Implemented theory through H3-W9b POWHEG real-smoke and real-free execution\par}
\vspace{0.5cm}
...
\end{equation}
This mode selects only the top-ranked candidate, writes one input card, executes
...
\code{ObserverBridge/}, \code{DIS/}, \code{ForwardGeodesics/}, or
\code{UHEsource/}.
...
candidates is set by \code{max_powheg_events}, and the number of independent
POWHEG hard-scattering realizations per selected interaction is set by
\code{events_per_candidate}. Each selected candidate is executed in its own
...
POWHEG Real Smoke & \code{ObserverBridge/observer_bridge_ranked_events.jsonl} &
