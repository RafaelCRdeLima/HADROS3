...
# Camera Preview PNG rows are stored with +e_theta at the top.  Since
# +e_theta points toward decreasing z (south), every Observer Overview image
# is displayed with a vertical flip so that -e_theta / +z / north is up.
# Pixel overlays must use the same transform as their background.
DEFAULT_OVERLAY_PIXEL_TRANSFORM = "flip_y"
...
def _camera_overlay_background_image(
    ...
    *,
    ...
    if image.shape[-1] == 4:
        image = image[..., :3]
    # The CUDA preview is saved south-up (+e_theta at the top).  Observer
    # Overview has a north-up display contract, so normalize the background
    # here once for every static and HTML figure generated by this stage.
    ...
    audit = {
        ...
        "background_transform_applied": "flip_y_for_north_up",
        "background_orientation": "north_up",
        "background_top_direction": "-e_theta / +z / NORTH",
        "background_bottom_direction": "+e_theta / -z / SOUTH",
...
def _write_background_audit(output_dir: Path, image: np.ndarray, audit: dict[str, Any]) -> dict[str, Any]:
    ...
    for ax, title in zip(axes, ["A: Camera Preview (north-up display)", "B: overlay background used (north-up)"], strict=True):
...
def _draw_overlay_position_scatter(
    ...
    *,
    ...
    if mode == "hemisphere":
        ...
        if handles:
            ax.legend(handles, legend_labels, loc="upper right", fontsize=8, framealpha=0.82)
...
def _multi_image_rows(
    ...
    *,
    ...
    metadata = {
        ...
        "image_clustering_model": "pixel_radius_connected_components",
        "image_clustering_order_dependent": False,
...
def _cluster_matching_rays(matches: list[dict[str, Any]], *, pixel_link_radius: float) -> list[dict[str, Any]]:
    ordered = sorted(
        ...
        key=lambda item: (
            ...
            float(item["pixel_y"]),
            int(item["ray_index"]),
            float(item["closest_approach_rg"]),
        ),
    )
    ...
    for left in range(len(ordered)):
        for right in range(left + 1, len(ordered)):
            distance = math.hypot(
                float(ordered[left]["pixel_x"]) - float(ordered[right]["pixel_x"]),
                float(ordered[left]["pixel_y"]) - float(ordered[right]["pixel_y"]),
            )
            if distance <= pixel_link_radius:
                adjacency[left].append(right)
                adjacency[right].append(left)
    clusters: list[list[dict[str, Any]]] = []
    visited: set[int] = set()
    for root in range(len(ordered)):
        if root in visited:
            continue
        stack = [root]
        visited.add(root)
        ...
        component.sort(key=lambda item: (int(item["ray_index"]), float(item["pixel_x"]), float(item["pixel_y"])))
        clusters.append(component)
    out: list[dict[str, Any]] = []
    for cluster_index, cluster in enumerate(clusters, start=1):
        best = min(cluster, key=lambda item: (float(item["closest_approach_rg"]), int(item["ray_index"])))
    ...
    out.sort(key=lambda item: (float(item["closest_approach_rg"]), tuple(item["ray_indices"])))
    ...
    *,
    ...
    if chosen:
        ...
        for index, cluster in enumerate(clusters):
            ...
            display_pixel = _camera_plane_to_overlay_image_pixel(float(pixel[0]), float(pixel[1]), width, height)
            if index == 0:
                color = "#22c55e"
                label = "primary image"
                size = 90
                ...
                size = 42
            ax.scatter([display_pixel[0]], [display_pixel[1]], s=size, color=color, edgecolors="white", linewidths=0.8, zorder=6, label=label)
            ax.text(float(display_pixel[0]) + 6, float(display_pixel[1]) - 6, str(index + 1), color="white", fontsize=8, weight="bold", bbox={"facecolor": "#020617", "alpha": 0.72, "edgecolor": "none", "pad": 2}, zorder=7)
        current = chosen.get("current_overlay_pixel")
        if current:
            display_current = _camera_plane_to_overlay_image_pixel(float(current[0]), float(current[1]), width, height)
            ax.scatter([display_current[0]], [display_current[1]], s=150, facecolors="none", edgecolors="#facc15", linewidths=2.2, zorder=8, label="current overlay choice")
        ax.legend(loc="upper right", fontsize=8, framealpha=0.82)
...
function draw() {{
  ...
  const displayY = y => data.height - 1 - Number(y);
  ...
  for (let i=0;i<clusters.length;i++) {{
    ...
    ctx.strokeStyle = '#fff';
    ctx.lineWidth = 1.4;
    ctx.arc(p[0], displayY(p[1]), i===0 ? 7 : 5, 0, Math.PI*2);
    ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 11px system-ui';
    ctx.fillText(String(i+1), p[0]+8, displayY(p[1])-8);
  }}
  if (c.current_overlay_pixel) {{
    const p = c.current_overlay_pixel;
    ctx.beginPath(); ctx.strokeStyle = '#facc15'; ctx.lineWidth = 3; ctx.arc(p[0], displayY(p[1]), 11, 0, Math.PI*2); ctx.stroke();
  }}
...
def _camera_basis_diagnostic(values: dict[str, dict[str, Any]], path: Path) -> dict[str, Any]:
    ...
    north_up_screen_direction = _vec_mul(e_theta, -1.0)
    interactive_previous_screen_up = e_theta
    interactive_screen_up = north_up_screen_direction
    payload = {
        ...
        "preview_up": list(north_up_screen_direction),
        ...
        "camera_preview_png_raw_top_direction": "+e_theta",
        "camera_preview_png_raw_bottom_direction": "-e_theta",
        "camera_preview_png_top_direction": "-e_theta",
        "camera_preview_png_bottom_direction": "+e_theta",
        "observer_overview_display_orientation": "north_up",
        "interactive_previous_screen_up_convention": "+e_theta (south-up)",
        "interactive_screen_up_convention": "-e_theta (north-up)",
        "interactive_matches_camera_preview": True,
        "basis_dot_products": {
            "forward_dot": _dot(forward, preview_forward),
            "right_dot": _dot(right, preview_right),
            "up_dot": _dot(interactive_screen_up, north_up_screen_direction),
            "previous_up_dot": _dot(interactive_previous_screen_up, north_up_screen_direction),
        },
        "right_dot": _dot(right, preview_right),
        "up_dot": _dot(interactive_screen_up, north_up_screen_direction),
        "previous_up_dot": _dot(interactive_previous_screen_up, north_up_screen_direction),
...
def _draw_camera_overlay(
    ...
    if primary_branch_path.exists():
        ...
        for index, row in enumerate(primary_rows):
            ...
            if pixel_x_value is None or pixel_y_value is None:
                continue
            display_x, display_y = _camera_plane_to_overlay_image_pixel(
                float(pixel_x_value), float(pixel_y_value), overlay_width, overlay_height
            )
            primary_projections.append(
                {
                    "pixel_x": display_x,
                    "pixel_y": float(pixel_y_value),
                    "pixel_y_image": display_y,
                    "inside": True,
                    "score": _score(row, "final_observation_score"),
    ...
    return {
        ...
        **overlay_metadata,
        ...
        "overlay_image_coordinate_transform": "flip_y_for_north_up",
        ...
        "candidate_overlay_y_axis_flipped_for_image": True,
...
def _write_kerr_interactive_view_html(
    ...
    screen_up = _vec_mul(_e_theta, -1.0)
    screen_right = e_phi
    ...
    data = {
        ...
        "camera": {
            ...
            "screen_up_convention": "-e_theta (north-up)",
            "screen_right_convention": "+e_phi",
            "camera_preview_png_top_direction": "-e_theta after north-up display flip",
            "camera_preview_png_bottom_direction": "+e_theta after north-up display flip",
            "interactive_matches_camera_preview": True,
