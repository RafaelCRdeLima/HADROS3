...
1. an existing conda/micromamba environment (``hadros3``, then the legacy
   ``dis`` used on the original development machine, then ``$CONDA_PREFIX``);
...
DEFAULT_ENV_NAME = os.environ.get("HADROS3_CONDA_ENV", "hadros3")
LEGACY_ENV_NAMES = ["hadros3", "dis"]
...
def resolve_existing_env(manager: str, requested: str) -> tuple[str, Path] | None:
    ...
    for name in [requested] + [n for n in LEGACY_ENV_NAMES if n != requested]:
...
def write_env_file(lines: list[str]) -> None:
    body = "\n".join(
        [
            "# Generated by `make setup` -- do not edit by hand, do not commit.",
...
def main() -> int:
    ...
    parser.add_argument("--env-name", default=DEFAULT_ENV_NAME, help="Conda environment name (default: hadros3).")
